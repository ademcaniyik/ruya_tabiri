...
if ($result->num_rows > 0) {
    ...
    if ($conn->query($insertUserSql) === TRUE) {
        ...
        if ($conn->query($insertTokenSql) === TRUE) {
            // Şimdi device_tokens tablosuna giriş ekle
            $insertDeviceTokenSql = "INSERT INTO device_tokens (user_id, device_token, created_at) VALUES ('$userId', '$deviceToken', '$createdAt')";
            ...
            if ($conn->query($insertDeviceTokenSql) === TRUE) {
                ...
                $response = [
                    'status' => false,
                    'message' => 'Kullanıcı ve token kaydedildi ancak cihaz token eklenirken bir hata oluştu.',
                    'parameters' => null
                ];
            }
        } else {
